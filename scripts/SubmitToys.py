...
def makeSubmitScript(toyDir, seed, n, wall):
    initDir=os.getcwd();
    dirname=toyDir
    os.chdir(dirname)
...
for toySeed in range(seedInput,numBatchJobs*numMCStudyToysPerBatchJob+seedInput,numMCStudyToysPerBatchJob):
    makeSubmitScript(toyDirName+ '/batch', str(toySeed), str(numMCStudyToysPerBatchJob),str(numWalltime)) 
...
makePlotScript(toyDirName+'/batch',str(numWalltime))
print 
